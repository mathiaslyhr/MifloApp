...
type Props = NativeStackScreenProps<RootStackParamList, 'OddOneOutPodium'>;
...
export function PodiumScreen({navigation, route}: Props) {
  ...
  const playAgain = useOddStore(s => s.playAgain);
  const hydrate = useOddStore(s => s.hydrate);
  ...
  useEffect(() => {
    ...
    return subscribeRoom(roomId, async room => {
      ...
      hydrate(
        room.questions as OddRound[],
        contestantsFromPlayers(roster, myUserId.current),
      );
      navigation.replace('OddOneOutQuestion', {roomId, code, isHost: false});
    });
  ...
  async function handlePlayAgain() {
    if (!roomId) {
      playAgain();
      navigation.replace('OddOneOutQuestion', {roomId, code, count: total});
      return;
    }
    ...
    try {
      ...
      const count = room?.questionCount ?? total ?? DEFAULT_ROUND_COUNT;
      const deck = buildRounds(count);
      ...
      navigation.replace('OddOneOutQuestion', {roomId, code, isHost: true});
    } catch {
