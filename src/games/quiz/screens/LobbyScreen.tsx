...
const SUBTITLE = `${DEFAULT_QUESTION_COUNT} questions · ${DEFAULT_TOPIC_IDS.length} topics`;
...
export function LobbyScreen({navigation, route}: Props) {
  const {code, isHost, name} = route.params;
  ...
  return (
    <Screen>
      ...
      <StickyFooter>
        <Button
          label="Start game"
          onPress={() => navigation.navigate('QuizQuestion', {code})}
        />
      </StickyFooter>
    </Screen>
  );
