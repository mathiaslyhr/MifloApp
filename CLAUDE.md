...
- **Player portraits**: when the user wants to generate/cut player images ("I'm going to start the images now"), follow `docs/portrait-batches.md` — 16-player ChatGPT sheets cut by `tools/art/slice-sheet.mjs grid16`, shipped OTA via `scripts/staging/portraits.json` + `data:publish`.
- **Check before adding — never create a second entry for an entity the data already knows.** Before adding a footballer, club, or Top Bins answer, search for it under other spellings (player surname; club short/long forms — 'Brøndby' vs 'Brøndby IF', 'Gladbach' vs 'Mönchengladbach'). If a similar entry exists, that IS the entry: extend it instead of adding a new one. A Top Bins answer that exists in the dataset must use the dataset's exact spelling as `name` (variants go in aliases) — otherwise the club shows up twice in the type-ahead. Tests enforce all of this (`integrity.test.ts`, `lists.test.ts`), so a duplicate fails `data:publish`; fix the duplicate rather than the test.
